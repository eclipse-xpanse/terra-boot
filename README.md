...
The AsyncApi docs can be accessed at http://localhost:9090/springwolf/docs.
The AsyncApi UI can be accessed at http://localhost:9090/springwolf/asyncapi-ui.html.

#### Generate AsyncApi Java Client

Use the following command to generate the AsyncApi Java Client:
```shell
curl -o asyncapi.json http://localhost:9090/springwolf/docs
## Todo: asyncapi 3.0.0 is not supported by the templates java-spring-template, we need to wait for the fix
npm install -g @asyncapi/cli @asyncapi/java-spring-template \
npx asyncapi generate fromTemplate asyncapi.json @asyncapi/java-spring-template \
-o ./src/main/java/org/eclipse/xpanse/terra/boot/queue/client \
-p binder=rabbit \
-p server=amqp-server \
-p javaPackage=org.eclipse.xpanse.terra.boot.queue.client \
--force-write
```
